...
import { Button, Dropdown, Space, Avatar, ConfigProvider, List } from "antd";
...
function NavBar() {
  ...
  const fetchNotifications = async () => {
    try {
      ...
      setUnreadCount(response.data.length); // Set the initial unread count
    } catch (error) {
  ...
  const showNotifications = () => {
    // Reset the unread count to 0 after showing notifications
    setUnreadCount(0);
  };

  const deleteAllNotifications = async () => {
    try {
      // Call the backend to delete all notifications
      ...
      {loggedUser && (
        <div className="Admin__NavBar">
          ...
          <Dropdown
            ...
            onOpenChange={(open) => {
              if (open) {
                showNotifications(); // Reset unread count when dropdown is opened
              }
            }}
          >
            <Badge
              count={unreadCount} // Show the unread count
            ...
            >
              ...
              >
                <Space>
                  <Avatar
                    size={30}
                    src={<img src={defaultProfilePic} alt="avatar" />}
                  />
                  {loggedUser.username}
